...
from tkinter import *
from backend_book import Database
...
class Window:
    ...
    def search_command(self):
        self.list1.delete(0, END)
        for row in database.search(self.title_text.get().title(), self.author_text.get().title(), self.year_text.get(), self.isbn_text.get()):
            self.list1.insert(END, row)

    def add_command(self):
        database.insert(self.title_text.get().title(), self.author_text.get().title(), self.year_text.get(), self.isbn_text.get())
        self.list1.delete(0, END)
    ...
    def update_command(self):
        database.update(selected_tuple[0],self.title_text.get().title(), self.author_text.get().title(), self.year_text.get(), self.isbn_text.get())
        self.view_command()
